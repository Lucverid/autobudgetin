...
(() => {
  ...
  let audioCtx = null, musicOn = false, musicTimer = null, masterGain = null, musicBus = null, compressor = null, loungeAudioActive = false, audioSession = 0;
  ...
  let thunderTimer = null, thunderEchoTimer = null, lightningClearTimer = null;
  ...
  const state = {
    ...
    cat: {
      ...
      facingLeft: true, mode: 'idle', seatUntil: 0, pauseUntil: 0, rainWatchUntil: 0, goal: 'wander', lastTick: 0,
      vx: 0, vy: 0, bob: 0
    },
  ...
  // Evaluasi harian hanya memberi bonus mood kecil setelah transaksi pertama tercatat.
  // Vitality/hearts internal khusus untuk lapar + revive, sedangkan hati yang tampil mengikuti financeHeartCap().
  // Penting: Rp0 di pagi/siang hari BUKAN "no-spend day" final, jadi jangan pernah auto-full hearts.
  function evaluateSpendProgress(){
    ...
    if (txCount <= 0) {
      state.pet.lastSpendNote = 'Belum ada transaksi hari ini. Status meong tetap mengikuti Daily Health secara live.';
      savePet();
      return;
    }
    ...
    let bonusMood = 0;
    if (snap.status === 'AMAN') bonusMood = 8;
    else if (snap.status === 'CUKUP') bonusMood = 4;

    if (bonusMood > 0) state.pet.mood = clamp(state.pet.mood + bonusMood, 0, 100);
    state.pet.lastSpendEvalDate = today;
    state.pet.lastSpendNote = `Transaksi pertama hari ini tercatat · ${fmtRp(total)} · ${snap.status}${bonusMood ? ` · mood +${bonusMood}` : ''}.`;
    savePet();
  }
  ...
  // Hati di HUD sekarang ikut mencerminkan kondisi keuangan, bukan cuma vitality pet.
  // Internal pet hearts tetap dipakai untuk hunger/revive; tampilan di-cap oleh status finansial.
  function financeHeartCap(){
    if (state.dailyFinanceStatus === 'END') return 0;
    if (state.dailyFinanceStatus === 'KRITIS') return 1;
    if (state.dailyFinanceStatus === 'WASPADA') return 3;
    if (state.dailyFinanceStatus === 'CUKUP') return 4;
    return MAX_HEARTS;
  }
  function displayedHearts(){
    return clamp(Math.min(Number(state.pet.hearts)||0, financeHeartCap()), 0, MAX_HEARTS);
  }
  ...
  function build(){
    ...
    el.innerHTML = `
      <div class="cat-lounge" role="dialog" aria-modal="true" aria-label="Cat After Hours">
        ...
        <div class="cat-scene" id="catScene">
          ...
          <div class="cat-rain" id="catRain" aria-hidden="true"></div>
          <div class="cat-lightning-flash" aria-hidden="true"></div>
          <div class="cat-window"><i></i><i></i><i></i><i></i><em></em><u></u><span class="window-rain-streaks" aria-hidden="true"></span><span class="window-droplets"><b></b><b></b><b></b><b></b><b></b><b></b><b></b><b></b></span></div>
    ...
    // Safety net: di luar lounge semua sumber suara harus berhenti seketika.
    // Context sengaja tidak di-close setiap keluar agar tidak ada race close/resume saat masuk lagi.
    const audioGuard = new MutationObserver(() => {
      if (!el.classList.contains('open') && (loungeAudioActive || musicOn || rainSource)) deactivateLoungeAudio();
    });
    audioGuard.observe(el, { attributes:true, attributeFilter:['class','aria-hidden'] });
    window.addEventListener('pagehide', finalDestroyAudio);
    window.addEventListener('beforeunload', finalDestroyAudio);
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        if (loungeAudioActive || musicOn || rainSource) deactivateLoungeAudio();
      } else if (el.classList.contains('open') && !loungeAudioActive) {
        // Kembali ke tab saat lounge masih terbuka: mulai sesi audio baru yang bersih.
        startMusic();
      }
    });
  ...
  function open(){
    ...
    startScene();
    // Setiap masuk lounge memulai sesi audio baru. Musik lobby default nyala lagi;
    // ambience hujan ikut state tombol hujan yang tersimpan.
    startMusic();
  }
  function close(){
    const ov = document.getElementById('catEggOverlay');
    ...
    // v25.5.16: UI harus SELALU bisa keluar, walaupun cleanup audio/scene error.
    // Tutup overlay lebih dulu agar satu exception tidak pernah mengunci user di lounge.
    ov.classList.remove('open');
    ov.setAttribute('aria-hidden', 'true');

    try { stopScene(); } catch (err) { console.warn('[Cat Lounge] stopScene cleanup:', err); }
    try { deactivateLoungeAudio(); } catch (err) { console.warn('[Cat Lounge] audio cleanup:', err); }
  }
  ...
  function stopScene(){
    ...
    clearTimeout(thunderEchoTimer); thunderEchoTimer = null;
    clearTimeout(lightningClearTimer); lightningClearTimer = null;
    cancelAnimationFrame(rafId); rafId = null;
    sceneEl?.classList.remove('lightning-flash');
    hideBubble();
  }
  ...
  function chooseWanderTarget(initial){
    ...
    if(!initial){
      const moods=[
        ...
        state.rainOn ? 'Si meong jalan santai sambil denger suara hujan dari jendela.' : 'Si meong jalan santai sambil denger ambience lobby malam.',
  ...
  function setRainWindowTarget(){
    if (!state.pet.alive || !state.rainOn || state.dailyFinanceStatus === 'KRITIS' || state.dailyFinanceStatus === 'END') return;
    const b = sceneBounds();
    resetPlayControls();
    state.cat.goal = 'rain-window';
    state.cat.targetX = clamp(b.left + Math.min(92, b.w * 0.18), b.left + 34, b.right - 20);
    state.cat.targetY = b.floor - 2;
    state.cat.rainWatchUntil = Date.now() + rand(4200, 7200);
    state.cat.pauseUntil = 0;
    setMood('Si meong berhenti dekat jendela, ngeliatin tetesan air yang ngalir di kaca.');
  }
  ...
  function sceneDecisionLoop(){
    ...
    if (Date.now() < state.cat.rainWatchUntil && state.cat.goal === 'rain-window') return;
    if (Date.now() < state.cat.pauseUntil) return;
    state.cat.pauseUntil = 0;

    // Hujan berada di luar. Tidak ada efek fisik "basah" atau slow khusus pada kucing indoor.
    // Satu-satunya reaksi yang logis: sesekali dia tertarik melihat kaca atau kaget saat guntur.
    if (state.rainOn && chance(0.34)) {
      setRainWindowTarget();
      return;
    }
  ...
  function checkIdleScene(){
    ...
    const sleepAfter = 18000;
    if (Date.now() - state.lastInteraction > sleepAfter && !state.toy.visible && !state.mouse.visible && state.cat.goal !== 'couch' && state.cat.goal !== 'rain-window') {
      state.cat.mode = 'sleep';
      setMood(state.rainOn ? 'Si meong ketiduran di dalam lobby sambil denger hujan di kaca.' : 'Si meong ketiduran pelan di lobby.');
  ...
  function reviveCat(){
    if(state.dailyFinanceStatus === 'END'){
      showBubble('pulihkan saldo');
      setMood('END itu kondisi dompet, bukan kematian pet. Tambah pemasukan / pulihkan saldo dulu, nanti meong otomatis balik.');
      return;
    }
    if(state.pet.alive){ showBubble('masih hidup'); setMood('Dia masih hidup, jadi Revive belum diperlukan.'); return; }
    if(state.pet.food < 2){
      showBubble('butuh 2 pakan');
      setMood('Revive sekarang butuh 2 pakan. Catat transaksi agar dapat reward pakan, lalu tekan Revive lagi.');
      return;
    }
    state.pet.food -= 2;
    state.pet.hearts = Math.max(2, Number(state.pet.hearts)||0);
    state.pet.hunger = 2;
    state.pet.alive = true;
    state.pet.mood = clamp(Math.max(state.pet.mood, 58), 0, 100);
    state.pet.lastDecayAt = Date.now();
    savePet();
    ...
    if (state.pet.alive && state.dailyFinanceStatus !== 'END') {
      ...
      if (!sleeping) {
        if (dist > 0.9) {
          ...
          } else if (state.cat.goal === 'rain-window') {
            state.cat.mode = 'idle';
            state.cat.facingLeft = true;
            state.cat.vx *= 0.72; state.cat.vy *= 0.72;
            if (Date.now() > state.cat.rainWatchUntil) {
              state.cat.rainWatchUntil = 0;
              if (state.rainOn && chance(0.5)) setCouchTarget(); else chooseWanderTarget(false);
            }
  ...
  function renderSceneClasses(){
    ...
    // Kucing ada di dalam lobby, jadi hujan tidak bikin dia otomatis basah.
    catEl.classList.remove('wet', 'rain-comfy');
  ...
  function renderHud(){
    if (!heartsEl) return;
    heartsEl.innerHTML = heartsMarkup(displayedHearts());
    heartsEl.setAttribute('aria-label', `Hati ${displayedHearts()} dari ${MAX_HEARTS} mengikuti kondisi finansial dan kesehatan meong`);
    ...
    if (financeEnded) {
      ...
      reviveBtn.textContent = '💸 Pulihkan saldo';
      reviveBtn.title = 'END adalah kondisi finansial. Meong kembali otomatis setelah saldo pulih.';
    } else if (state.pet.alive) {
      reviveBtn.disabled = true;
      reviveBtn.textContent = '💖 Revive';
      reviveBtn.title = 'Revive aktif kalau meong tumbang karena kelaparan.';
    } else {
      reviveBtn.disabled = false;
      reviveBtn.textContent = state.pet.food >= 2 ? '💖 Revive · 2 pakan' : '💖 Revive · butuh 2 pakan';
      reviveBtn.title = 'Gunakan 2 pakan untuk menghidupkan meong lagi.';
    }
  ...
  function overlayIsOpen(){
    return !!document.getElementById('catEggOverlay')?.classList.contains('open');
  }

  function stopRainSource(){
    if(rainSource){
      try{ rainSource.stop(0); }catch{}
  ...
  function silenceMusicNodes(){
    clearInterval(musicTimer); musicTimer = null;
    activeNodes.forEach(n=>{ try{n.stop(0)}catch{} try{n.disconnect?.()}catch{} });
    activeNodes = [];
  ...
  function deactivateLoungeAudio(){
    // v25.5.15: hard-silence sinkron. Tidak ada fade, timeout, suspend, atau close saat keluar.
    // Dengan begitu hujan dan musik berhenti pada event close yang sama dan tidak berebut
    // AudioContext ketika lounge dibuka kembali.
    audioSession += 1;
    loungeAudioActive = false;
    musicOn = false;
    clearTimeout(thunderTimer); thunderTimer = null;
    clearTimeout(thunderEchoTimer); thunderEchoTimer = null;
    clearTimeout(lightningClearTimer); lightningClearTimer = null;
    ...
    if(musicBtn){
      // Saat masuk lagi musik memang dimulai otomatis, jadi tombol kembali ke state default on.
      musicBtn.classList.add('active');
      musicBtn.textContent = '♫ Lobby on';
    }
  }

  function finalDestroyAudio(){
    // Hanya untuk pagehide/unload. Keluar lounge biasa memakai deactivateLoungeAudio().
    deactivateLoungeAudio();
    if(!audioCtx) return;
    const ctx = audioCtx;
  ...
  async function startMusic(){
    ...
    loungeAudioActive = true;
    musicOn = true;
    if(musicBtn){ musicBtn.classList.add('active'); musicBtn.textContent='♫ Lobby on'; }
    try{
      ensureAudio();
      const ctx = audioCtx;
      if(ctx.state !== 'running') await ctx.resume();
      // Cegah callback async dari sesi lama menghidupkan audio lagi setelah close/reopen cepat.
      if(mySession !== audioSession || !loungeAudioActive || !overlayIsOpen() || audioCtx !== ctx) return;

      const targetGain = 1.8 * (state.volume/100);
      masterGain.gain.cancelScheduledValues(ctx.currentTime);
      masterGain.gain.setValueAtTime(targetGain, ctx.currentTime);
      musicBus.gain.cancelScheduledValues(ctx.currentTime);
      musicBus.gain.setValueAtTime(1.28, ctx.currentTime);
      applyVolumeValue(state.volume);

      silenceMusicNodes();
      // silenceMusicNodes men-zero-kan musicBus, jadi pulihkan tepat sebelum scheduling.
      musicBus.gain.setValueAtTime(1.28, ctx.currentTime);
      musicOn = true;
      scheduleLoop();
      musicTimer = setInterval(()=>{
        if(loungeAudioActive && musicOn && overlayIsOpen()) scheduleLoop();
      }, Math.round((60/74)*4*4*1000));

      // Rain ambience independen dari musik lobby. Kalau setting hujan ON, ia ikut hidup
      // hanya selama lounge terbuka. Guntur baru dijadwalkan SETELAH audio session valid;
      // sebelumnya startScene() memanggil scheduler terlalu dini saat loungeAudioActive masih false.
      setRainAudio(state.rainOn);
      if (state.rainOn) scheduleNextThunder();
      else { clearTimeout(thunderTimer); thunderTimer = null; }
    }catch(err){
      console.warn('[Cat Lounge] startMusic failed:', err);
      if(mySession === audioSession){
        ...
        sceneEl?.classList.remove('lightning-flash');
        if(audioCtx && masterGain){
          try{ masterGain.gain.cancelScheduledValues(audioCtx.currentTime); masterGain.gain.setValueAtTime(0, audioCtx.currentTime); }catch{}
        }
        if(musicBtn){ musicBtn.classList.remove('active'); musicBtn.textContent='♫ Lobby off'; }
      }
    }
  }

  function stopMusic(){
    // Tombol Lobby off hanya mematikan MUSIK, bukan ambience hujan.
    musicOn = false;
    silenceMusicNodes();
    if(musicBtn){ musicBtn.classList.remove('active'); musicBtn.textContent='♫ Lobby off'; }
  }
  ...
  function createRainNoise(){
    if(!audioCtx || rainSource || !loungeAudioActive || !overlayIsOpen()) return;
  ...
  function setRainAudio(on){
    ...
    if(!on || !loungeAudioActive || !overlayIsOpen()){
      stopRainSource();
      return;
    }
    createRainNoise();
    ...
    try{
      rainGain.gain.cancelScheduledValues(audioCtx.currentTime);
      // tidak pakai fade panjang supaya status ambience selalu deterministik saat buka/tutup.
      rainGain.gain.setValueAtTime(.12, audioCtx.currentTime);
    }catch{}
  }
  ...
  function thunderRumble(){
    if(!audioCtx || !loungeAudioActive || !state.rainOn || !masterGain || !overlayIsOpen()) return;
    ...
    lp.type = 'lowpass'; lp.frequency.value = 170; lp.Q.value = .7;
    g.gain.setValueAtTime(.0001, now);
    g.gain.exponentialRampToValueAtTime(.16, now + .05);
    g.gain.exponentialRampToValueAtTime(.0001, now + dur);
    src.buffer = buf; src.connect(lp).connect(g).connect(masterGain);
    src.start(now); src.stop(now+dur+.05); remember(src);
  }

  function catReactToThunder(){
    if(!state.pet.alive || state.dailyFinanceStatus === 'END') return;
    if(state.dailyFinanceStatus === 'KRITIS'){
      showBubble('!');
      setMood('Guntur kedengeran dari luar. Meong cuma melirik dari sofa, masih bad mood.');
      return;
    }
    state.cat.vx = 0; state.cat.vy = 0;
    state.cat.mode = 'idle';
    state.cat.pauseUntil = Date.now() + rand(700, 1200);
    showBubble(chance(.5) ? '!' : 'mrrp?!');
    if(chance(.48)) setTimeout(()=>{ if(state.rainOn && canPlayNow() && !state.toy.visible && !state.mouse.visible) setCouchTarget(); }, 900);
  }

  function triggerThunder(){
    const ov = document.getElementById('catEggOverlay');
    if(!state.rainOn || !sceneEl || !ov?.classList.contains('open')) return;
    sceneEl.classList.remove('lightning-flash');
    void sceneEl.offsetWidth;
    sceneEl.classList.add('lightning-flash');
    const delay = Math.round(rand(260, 820));
    clearTimeout(thunderEchoTimer);
    thunderEchoTimer = setTimeout(()=>{
      const nowOv = document.getElementById('catEggOverlay');
      if(state.rainOn && loungeAudioActive && nowOv?.classList.contains('open')){
        thunderRumble(); catReactToThunder();
      }
    }, delay);
    clearTimeout(lightningClearTimer);
    lightningClearTimer = setTimeout(()=>sceneEl?.classList.remove('lightning-flash'), 1150);
  }

  function scheduleNextThunder(){
    clearTimeout(thunderTimer);
    if(!state.rainOn || !loungeAudioActive || !overlayIsOpen()) return;
    thunderTimer = setTimeout(()=>{
      triggerThunder();
      scheduleNextThunder();
    }, Math.round(rand(12000, 26000)));
  }
  function toggleRain(){
    state.rainOn = !state.rainOn;
    localStorage.setItem('agis_cat_lobby_rain', state.rainOn ? '1' : '0');
    updateRainUi(); renderSceneClasses(); setRainAudio(state.rainOn);
    if (state.rainOn) {
      setMood('Hujan ada di luar: tetesan di kaca, kota lebih redup, plus kilat dan guntur sesekali. Meong tetap kering di dalam.');
      showBubble('hujan di luar...');
      scheduleNextThunder();
      if (canPlayNow() && !state.toy.visible && !state.mouse.visible && chance(0.52)) setRainWindowTarget();
    } else {
      clearTimeout(thunderTimer); thunderTimer = null;
      clearTimeout(thunderEchoTimer); thunderEchoTimer = null;
      clearTimeout(lightningClearTimer); lightningClearTimer = null;
      sceneEl?.classList.remove('lightning-flash');
      state.cat.rainWatchUntil = 0;
      if (state.cat.goal === 'rain-window') chooseWanderTarget(false);
      setMood('Hujan berhenti. Kaca perlahan bening dan suasana luar kembali normal.');
      showBubble('reda~');
    }
  }
