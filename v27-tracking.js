(() => {
  ...
  function renderSalesCalendar(b, month, selected) {
    ...
    for (let day = 1; day <= count; day++) {
      const date = `${month}-${String(day).padStart(2, '0')}`, d = dailyStats(b, date), has = d.qty > 0 || d.revenue > 0, ratio = has ? d.qty / Math.max(1, planned || maxQty) : 0, heat = ratio >= 1.5 ? 4 : ratio >= 1 ? 3 : ratio >= .5 ? 2 : has ? 1 : 0;
