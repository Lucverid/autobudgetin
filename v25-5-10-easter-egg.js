...
(() => {
  ...
  function setRainWindowTarget(){
    ...
    state.cat.pauseUntil = 0;
    setMood('Hujan turun. Si meong cari spot kering lalu ngeliatin tetesan di kaca.');
  }
  ...
  function sceneDecisionLoop(){
    ...
    // Saat hujan, kucing indoor tidak otomatis 'basah'. Dia lebih realistis:
    // lebih kalem, lebih sering cari sofa/hangat, atau menonton tetesan dari dekat jendela.
    if (state.rainOn) {
      const rainyCouch = state.dailyFinanceStatus === 'WASPADA' ? 0.52 : state.dailyFinanceStatus === 'CUKUP' ? 0.38 : 0.30;
      if (chance(rainyCouch)) setCouchTarget();
      else if (chance(0.62)) setRainWindowTarget();
  ...
  function checkIdleScene(){
    ...
    const sleepAfter = state.rainOn ? 12500 : 18000;
    if (Date.now() - state.lastInteraction > sleepAfter && !state.toy.visible && !state.mouse.visible && state.cat.goal !== 'couch' && state.cat.goal !== 'rain-window') {
      ...
      setMood(state.rainOn ? 'Suara hujan bikin si meong lebih cepat ngantuk di tempat yang hangat.' : 'Si meong ketiduran pelan di lobby.');
  ...
  function tick(ts){
    ...
    if (state.pet.alive && state.dailyFinanceStatus !== 'END') {
      ...
      const rainMul = state.rainOn && state.cat.goal !== 'mouse' && state.cat.goal !== 'toy' ? 0.86 : 1;
      const speed = (state.cat.goal === 'mouse' ? state.cat.chaseSpeed : state.cat.goal === 'toy' ? state.cat.chaseSpeed * 0.88 : state.cat.speed) * behaviorMul * rainMul;
  ...
  function renderSceneClasses(){
    ...
    catEl.classList.remove('wet');
    catEl.classList.toggle('rain-comfy', state.rainOn && state.pet.alive && state.dailyFinanceStatus !== 'END');
  ...
  function toggleRain(){
    ...
    if (state.rainOn) {
      setMood('Hujan aktif. Meong tetap kering di dalam lobby, jadi lebih kalem dan suka ngeliatin tetesan kaca.');
      showBubble('hujan...');
      if (canPlayNow() && !state.toy.visible && !state.mouse.visible && chance(0.72)) setRainWindowTarget();
    } else {
      state.cat.rainWatchUntil = 0;
      if (state.cat.goal === 'rain-window') chooseWanderTarget(false);
      setMood('Hujan berhenti. Meong mulai keliling lobby lagi.');
      showBubble('cerah~');
    }
  }
