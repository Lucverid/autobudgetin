...
(() => {
  ...
  function stopMusic(){
    ...
    if(audioCtx){
      try{
        setRainAudio(false);
        masterGain?.gain.cancelScheduledValues(audioCtx.currentTime);
        masterGain?.gain.setTargetAtTime(.0001,audioCtx.currentTime,.035);
      }catch{}
    }
  }
  ...
  function triggerThunder(){
    if(!state.rainOn || !sceneEl) return;
    sceneEl.classList.remove('lightning-flash');
    void sceneEl.offsetWidth;
    sceneEl.classList.add('lightning-flash');
    const delay = Math.round(rand(260, 820));
    setTimeout(()=>{ if(state.rainOn){ thunderRumble(); catReactToThunder(); } }, delay);
    setTimeout(()=>sceneEl?.classList.remove('lightning-flash'), 1150);
  }
