(() => {
  ...
  function spendingContext(){
    const today=dateKey(), month=today.slice(0,7), now=new Date();
    const floor=Math.max(0,Number(smartSettings().safeFloor)||0);
  ...
  function init(){inject();wrapPersistence();if(settings().autoSync&&settings().backendUrl)setTimeout(()=>syncNow(true),4200);}
