...
(() => {
  ...
  function liveFinanceSnapshot(){
    ...
    return { spent, safeLimit, score, status: dailyStatusFromScore(score) };
  }
  ...
  function refreshDailyFinanceStatus(announce = false){
    ...
    if (announce && prev && prev !== snap.status && document.getElementById('catEggOverlay')?.classList.contains('open')) {
      if (snap.status === 'KRITIS') setMood('Pengeluaran hari ini masuk KRITIS. Si meong langsung bad mood.');
      else if (snap.status === 'WASPADA') setMood('Pengeluaran hari ini mulai WASPADA. Si meong jadi lebih pendiam.');
      else if (snap.status === 'CUKUP') setMood('Pengeluaran hari ini masih CUKUP. Mood si meong mulai tenang.');
      else setMood('Pengeluaran hari ini kembali AMAN. Si meong kelihatan lebih lega.');
    }
  ...
  function effectiveMood(){
    ...
    if (state.dailyFinanceStatus === 'KRITIS') return Math.min(base, 24);
  ...
  function hideToy(){ state.toy.visible = false; state.toy.reached = false; state.toy.lastReactAt = 0; toyEl?.classList.add('hidden'); if (state.cat.goal === 'toy') chooseWanderTarget(false); }
