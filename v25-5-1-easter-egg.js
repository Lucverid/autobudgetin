...
(() => {
  ...
  const state = {
    ...
    cat: {
      x: 88, y: 238, targetX: 88, targetY: 238, speed: 66, chaseSpeed: 154,
      facingLeft: true, mode: 'idle', seatUntil: 0, goal: 'wander', lastTick: 0
    },
    mouse: { visible: false, x: 260, y: 238, vx: 24, phase: 0 },
      ...
      <div class="cat-lounge" role="dialog" aria-modal="true" aria-label="Cat After Hours">
        <div class="cat-lounge-head">
          <div>
            ...
            <div class="cat-lounge-sub">Meong sekarang punya ekspresi wajah yang ikut kondisi keuangan harian: bisa happy, santai, bad mood, sampai lesu — tapi tetap bisa diajak main dan dirawat.</div>
          </div>
        ...
        <div class="cat-scene" id="catScene">
          ...
          <button class="cat-avatar" id="catPet" aria-label="Pat pat kucing">
            ...
            <span class="cat-brow left" aria-hidden="true"></span>
            <span class="cat-brow right" aria-hidden="true"></span>
            <span class="cat-mouth" aria-hidden="true"></span>
            <span class="cat-cheek left" aria-hidden="true"></span>
            <span class="cat-cheek right" aria-hidden="true"></span>
  ...
  function sceneBounds(){
    ...
    return { w, h, left:20, right:Math.max(20, w-112), floor:Math.max(140, h-88), couchX:w-118, couchY:Math.max(120, h-160) };
  }
  ...
  function setCouchTarget(){
    ...
    state.cat.seatUntil = Date.now() + 4800;
    setMood('Si meong naik ke sofa dulu, duduk santai kayak pemilik lobby.');
  }
  function chooseWanderTarget(initial){
    ...
    state.cat.targetY = rand(b.floor-6, b.floor+2);
    if(!initial){
      const moods=[
  ...
  function spawnMouse(manual){
    ...
    state.mouse.vx = Math.random()>.5 ? 22 : -22;
    state.mouse.phase = 0;
  ...
  function updateMouse(dt){
    ...
    const b=sceneBounds();
    state.mouse.phase += dt * 0.005;
    state.mouse.x += state.mouse.vx * dt * 0.001;
    state.mouse.y = b.floor + Math.sin(state.mouse.phase * 6) * 2.4;
    if (state.mouse.x < b.left + 12 || state.mouse.x > b.w - 50) state.mouse.vx *= -1;
  }
  ...
  function tick(ts){
    ...
    if (state.pet.alive) {
      if (state.mouse.visible) {
        ...
        state.cat.targetY = clamp(state.toy.y + 20, b.floor - 10, b.floor + 4);
      }
      ...
      const speed = state.cat.goal === 'mouse' ? state.cat.chaseSpeed : state.cat.goal === 'toy' ? state.cat.chaseSpeed * 0.92 : state.cat.speed;

      if (!sleeping && dist > 1.1) {
        state.cat.x += dx / dist * speed * dt * 0.001;
        state.cat.y += dy / dist * speed * dt * 0.001;
        if (Math.abs(dx) > 1.5) state.cat.facingLeft = dx < 0;
        if (state.cat.goal === 'mouse') state.cat.mode = 'run';
        else if (state.cat.goal === 'toy') state.cat.mode = 'play';
        else if (state.cat.goal === 'couch') state.cat.mode = 'walk';
        else state.cat.mode = 'walk';
      } else {
        if (state.cat.goal === 'mouse' && state.mouse.visible) {
          showBubble('gotcha!');
          burst(state.cat.x + 34, state.cat.y - 10, 'tap!');
          setMood('Si meong hampir nangkap tikus. Lucu banget gayanya.');
          if (Math.abs(state.cat.x - state.mouse.x) < 22) {
            hideMouse(true);
            setMood('Tikus ketangkep. Si meong bangga sendiri.');
            chime();
          }
        } else if (state.cat.goal === 'toy' && state.toy.visible) {
          state.cat.mode = 'play';
          if (Math.abs(state.cat.x - state.toy.x) < 14) {
            burst(state.cat.x + 34, state.cat.y - 10, 'hehe!');
            state.toy.bounce = 1;
          }
  ...
  function renderActors(){
    if (catEl) {
      ...
      catEl.style.transform = `translate(${state.cat.x}px, ${state.cat.y}px) scaleX(${flip})`;
      ...
      const screenGX = clamp((focusX - state.cat.x) / 90, -1, 1) * 1.8;
      const screenGY = clamp((focusY - state.cat.y) / 70, -1, 1) * 1.0;
