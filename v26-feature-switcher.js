(() => {
  ...
  const FEATURES=[
    ...
    {id:'tracking',label:'Realisasi',icon:'activity',desc:'Pantau penjualan, stok, profit, dan cicilan.'},
  ...
  let active=localStorage.getItem(KEY)||'decision';
  if(!FEATURES.some(x=>x.id===active))active='decision';
  let attempts=0, timer=null, navWrapped=false;
  ...
  function nodes(){
    return {
      ...
      report:document.querySelector('.v25-year-card')
    };
  }
  ...
  function makeSheet(){
    ...
    overlay.id='v26fs-sheet';
    overlay.className='v26fs-sheet';
    overlay.hidden=true;
  ...
  function arrange(){
    ...
    const n=nodes();
    // Pindahkan ringkasan planning dan laporan dari Home ke halaman Planning sekali saja.
    if(n.budget&&n.budget.parentElement!==h)h.insertBefore(n.budget,h.firstChild);
    const bar=makeSwitcher();
    if(n.budget&&bar&&bar.previousElementSibling!==n.budget)h.insertBefore(bar,n.budget.nextSibling);
    if(n.report&&n.report.parentElement!==h)h.appendChild(n.report);
    const loading=qs('.v2531-planning-loading',h);if(loading)loading.remove();
    makeSheet();
    apply(active,false);
    return !!(n.lab&&n.whatif);
  }
  ...
  function apply(id,scroll=true){
    ...
    saveActive(id);
    const n=nodes(), lab=n.lab, tracking=n.tracking;
    const showLab=id==='decision'||id==='tracking';
    setVisible(lab,showLab);setVisible(n.whatif,id==='whatif');setVisible(n.report,id==='report');

    if(lab){
      lab.classList.toggle('v26fs-tracking-mode',id==='tracking');
      if(tracking){
        tracking.hidden=id!=='tracking';
        if(id==='tracking'){
          tracking.open=true;
          setTimeout(()=>window.refreshStableTracking?.(),0);
        }else if(tracking.open){tracking.open=false;}
      }
    }

    document.querySelectorAll('[data-v26fs]').forEach(btn=>{
      const on=btn.dataset.v26fs===id;
      btn.classList.toggle('active',on);btn.setAttribute('aria-selected',on?'true':'false');
      if(on&&scroll)btn.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});
    });
    const cur=document.getElementById('v26fs-current'),f=FEATURES.find(x=>x.id===id);if(cur&&f)cur.textContent=f.label;
    if(id==='tracking'&&!tracking){clearTimeout(timer);timer=setTimeout(()=>{arrange();apply('tracking',false)},260);}
    if(window.lucide?.createIcons)lucide.createIcons();
  }
  ...
  function wrapNav(){
    ...
    window.nav=function(id,el){const out=orig.apply(this,arguments);if(id==='planning')setTimeout(()=>{arrange();apply(active,false)},90);return out;};
    navWrapped=true;
  }
  ...
  function boot(){
    wrapNav();
    if(arrange())return;
    if(attempts++<16)setTimeout(boot,220);
  }
