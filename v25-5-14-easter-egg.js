...
(() => {
  ...
  let audioCtx = null, musicOn = false, musicTimer = null, masterGain = null, musicBus = null, compressor = null, audioSuspendTimer = null;
  ...
  function build(){
    ...
    // Safety net: kalau overlay ditutup oleh perubahan DOM/navigasi selain tombol X,
    // audio tetap dimatikan total.
    const audioGuard = new MutationObserver(() => {
      if (!el.classList.contains('open') && audioCtx) destroyAudio();
    });
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.hidden && !el.classList.contains('open') && audioCtx) destroyAudio();
    });
  ...
  function open(){
    build();
    // Batalkan suspend tertunda dari close sebelumnya. Ini penting kalau lounge dibuka lagi cepat,
    // terutama saat mode hujan masih tersimpan aktif.
    clearTimeout(audioSuspendTimer); audioSuspendTimer = null;
    advancePetClock();
    syncFoodReward();
  ...
  function close(){
    ...
    stopScene();
    // v25.5.14: keluar lounge = hancurkan seluruh audio graph, bukan cuma mute/suspend.
    // Ini menjamin tidak ada oscillator/rain loop/guntur yang bisa bocor ke halaman utama.
    destroyAudio();
  }
  ...
  function scheduleAudioSuspend(delay=320){
    clearTimeout(audioSuspendTimer);
    if(!audioCtx) return;
    audioSuspendTimer = setTimeout(async()=>{
      const ov = document.getElementById('catEggOverlay');
      if(musicOn || ov?.classList.contains('open')) return;
  ...
  async function startMusic(){
    ...
    try{
      ...
      if(!musicOn || !ov.classList.contains('open')) return;
      // stopMusic mem-fade masterGain ke hampir nol. Saat lounge dibuka lagi gain wajib
      // dipulihkan SETELAH AudioContext benar-benar running, bukan sebelum resume selesai.
  ...
  function stopMusic(hard = false){
    ...
    if(audioCtx){
      try{
        // Rain ambience juga bagian dari lounge, jadi ikut diam saat lounge ditutup.
        if(rainGain){
        ...
        if(masterGain){
          masterGain.gain.cancelScheduledValues(audioCtx.currentTime);
          if(hard) masterGain.gain.setValueAtTime(0, audioCtx.currentTime);
          else masterGain.gain.setTargetAtTime(.0001,audioCtx.currentTime,.025);
        }
      }catch{}
    }
  }
  ...
  function destroyAudio(){
    // Full teardown. Dipakai saat lounge benar-benar ditutup / halaman ditinggalkan.
    musicOn = false;
    clearInterval(musicTimer); musicTimer = null;
    clearTimeout(audioSuspendTimer); audioSuspendTimer = null;
    clearTimeout(thunderTimer); thunderTimer = null;
    ...
    if(audioCtx){
      const ctx = audioCtx;
      try{
        if(rainGain){ rainGain.gain.cancelScheduledValues(ctx.currentTime); rainGain.gain.setValueAtTime(0,ctx.currentTime); }
        if(masterGain){ masterGain.gain.cancelScheduledValues(ctx.currentTime); masterGain.gain.setValueAtTime(0,ctx.currentTime); }
      }catch{}
    ...
    if(musicBtn){
      // Tombol tetap default ON secara UX; saat masuk lagi open() akan start fresh.
  ...
  function triggerThunder(){
    ...
    thunderEchoTimer = setTimeout(()=>{
      ...
      if(state.rainOn && musicOn && nowOv?.classList.contains('open')){
